...
::: {.hero-sub}
Esta pieza parte del diseño canónico de dos grupos y dos períodos y avanza hacia una versión condicional que reconstruye la población de comparación mediante propensity scores y soporte común.
:::
:::
...
La ponderación de Abadie utiliza el propensity score, $p(X)=P(D=1\mid X)$, para cambiar la composición del grupo de control. Los controles que representan mejor a la población tratada reciben mayor peso; los perfiles con escasa comparabilidad reciben menos influencia.
...
![Distribución del propensity score para participantes y controles NSW](did01_overlap_nsw.svg){fig-alt="Histogramas superpuestos del propensity score para tratados y controles. La mayor parte de ambas distribuciones se encuentra dentro de la intersección de soporte entre 0,239 y 0,629."}

Los propensity scores presentan una intersección amplia. La versión operativa del gráfico, estimada mediante logit con las covariables predeterminadas, ubica el rango común aproximadamente entre `0,239` y `0,629`. El diagnóstico original con probit arroja una conclusión prácticamente idéntica. La estabilidad del ATT al restringir el soporte indica que el resultado no depende materialmente de unas pocas observaciones sin comparación adecuada.
...
## Puente hacia diseños DiD con más períodos

El DiD canónico establece la idea fundamental: identificar mediante una diferencia de cambios bajo tendencias paralelas. La versión condicional muestra que esa comparabilidad puede construirse con observables, weighting y soporte común.

El paso siguiente es estudiar qué ocurre cuando se dispone de muchos períodos. Antes de reconsiderar el estimando, aparece un problema clásico de inferencia: los outcomes y los tratamientos pueden persistir en el tiempo, de modo que errores estándar ingenuos tratan como independiente información que no lo es. Esa cuestión requiere una pieza propia.
...
La contribución central no se reduce a esos dos números. La pieza muestra una progresión: tendencias paralelas incondicionales en el diseño canónico, tendencias paralelas condicionales cuando la composición importa y ponderación con soporte común para construir una comparación más pertinente. Esa arquitectura es el punto de partida para estudiar después la inferencia en paneles largos y, más adelante, los problemas de los diseños DiD con adopción escalonada.
