...
El último experimento compara cinco métodos sobre la misma ley placebo en cada repetición externa y agrega un block bootstrap que remuestrea trayectorias estatales completas. Con 75 repeticiones externas y 74 remuestreos internos, OLS rechaza 38,7% bajo el nulo; cluster, pre-post y la matriz empírica común rechazan 6,7%, y el block bootstrap, 5,3%.

El bootstrap queda cerca del tamaño nominal, pero el número reducido de repeticiones impide convertir diferencias pequeñas en una jerarquía definitiva. Su aporte es mostrar que preservar la trayectoria completa del estado puede corregir la falsa precisión sin tratar cada celda estado-año como información independiente.

## Mismo coeficiente, distinta incertidumbre

![Una misma ley placebo produce el mismo coeficiente y errores estándar distintos según el método de inferencia](bdm01_mismo_coeficiente.svg){fig-alt="El coeficiente estimado es 0,0057 con los cinco métodos. El error estándar es 0,0058 con OLS convencional, 0,0128 con cluster por estado, 0,0123 con matriz empírica común, 0,0080 con AR uno estimado y 0,0179 con AR uno imponiendo rho igual a 0,8."}

En una ley placebo fija, los cinco métodos producen exactamente el mismo coeficiente: `0,0057`. Lo que cambia es la incertidumbre. El error estándar convencional (`0,0058`) es menos de la mitad del cluster por estado (`0,0128`). La figura aísla así el mecanismo del problema: no aparecen cinco efectos causales distintos, sino cinco evaluaciones de precisión para un único estimador puntual.

Tampoco debe leerse la barra más larga como el “mejor” método. Imponer `ρ = 0,8` eleva el error estándar a `0,0179` y puede volver la inferencia excesivamente conservadora. La evidencia favorece reconocer la dependencia pertinente, no maximizar mecánicamente la incertidumbre.
