...
Regla de sobriedad común:

- gráficos chicos o medianos, no protagonistas
- evitar estética de infografía vistosa
- evitar leyendas/notas al pie innecesarias
- priorizar 1-2 tonos neutros del portfolio

## Lenguaje visual y categorías públicas

- En SVG/tarjetas, escribir para lector externo; evitar jerga de cocina econométrica como texto principal.
- En tablas y visuales, mostrar primero el significado conceptual de categorías; no códigos internos del do-file como primera capa (`0/1`, `choice=1`, etc.).
- Si una codificación numérica tiene sentido sustantivo, acompañarla por nombre conceptual.
- Regla práctica de legibilidad: el visual debe decir qué se compara, qué cambia y qué decisión deja.

## Codificación y formato de assets

- Revisar tildes/eñes y español visible dentro de cada SVG antes de cerrar.
- Mantener `.qmd` en UTF-8 correcto (sin BOM cuando corresponda).
- Si aparecen secuencias tipo `Ã`, `Â`, `â€™`, corregir antes de cerrar.
- `.gph` es fuente editable de Stata, no formato de publicación web.
- Para web: preferir SVG simple y liviano; si un SVG de Stata es pesado, usar PNG.

## Reglas visuales consolidadas por caso (causalidad)

### c02 / T2 (matching exacto)

- Priorizar un único gráfico de balance como apoyo visual (`3_smd.svg`).
- El foco es sostener criterio de selección, no llenar la pieza de figuras.

### c04 / T4 (PSM)

- El visual útil central es el love plot.
- No forzar overlap base si no conversa con la discusión principal.

### c06 / T6 (IPWRA)

- Priorizar el par before/after del contraste más exigente (`3 vs 0`) para mostrar efecto de ponderación.
- Evitar saturación con múltiples gráficos secundarios.

### c08 / T8 (IV en causalidad)

- Sí pertenece plenamente a causalidad por su estrategia de identificación.
- La lógica causal proviene de variación exógena inducida por instrumentos.
- No hay tratados/controles estándar, pero sí estrategia causal.
- No diluir causalidad solo por debilidad de instrumentos.
- Tampoco volver la pieza puramente técnica.
- Si los SVG de Stata son pesados, reemplazarlos por PNG.

Regla técnica aprendida en `c08`:

- el problema observado fue peso y complejidad extrema del SVG exportado
- la solución correcta fue PNG liviano para publicación web

## Regla narrativa para piezas IV en causalidad

- Cuando el diseño IV lo justifica, la pieza puede y debe hablar de causalidad con sobriedad.
- Debilidad de instrumentos limita credibilidad empírica, pero no borra la naturaleza causal de la estrategia.
- No centrar la historia en “tests, tests, tests” si eso eclipsa la lógica causal.
...
## Sincronización entre árboles de causalidad

Si la pieza existe en ambos árboles (`causalidad/` y `portfolio-site/causalidad/`):

- mantener assets visuales sincronizados cuando corresponda
- mantener `.qmd` coherentes entre ambas rutas
- renderizar HTML en ambos árboles
- verificar actualización efectiva en portal
